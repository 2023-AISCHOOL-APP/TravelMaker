      ...
      <div>
          <div className='my-schedule-list'>
              ...
              <div className="apllicant-box">
                ...
                <>
                  {selectedApp.map((id) => {
                      return (<div className='applicant' onChange={(e)=>{if(applicantList.indexOf(e.target.value)===-1){setApplicantList([...applicantList, e.target.value])}else{setApplicantList(applicantList.filter(user => user != e.target.value))}}}><input type='checkbox' className='applicant-check' value={id}></input>
                            <div>{id}</div></div>)
                  })}
                  </>
                  }
